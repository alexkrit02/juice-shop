...
describe('CodeSnippetService', () => {
  ...
  it('should get single snippet directly from the rest api', 
    inject([CodeSnippetService, HttpTestingController], 
      fakeAsync((service: CodeSnippetService, httpMock: HttpTestingController) => {
        let res: { snippet: string };
        service.get('testChallenge').subscribe((data: { snippet: string }) => (res = data));
  ...
  it('should get list of challenges with code snippets directly from the rest api', 
    inject([CodeSnippetService, HttpTestingController], fakeAsync((service: CodeSnippetService, httpMock: HttpTestingController) => {
      let res: string[]; 
      service.challenges().subscribe((data: string[]) => (res = data));
